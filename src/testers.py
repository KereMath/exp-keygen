"""
İstatistiksel test modülü - Recursion hatası düzeltildi
"""
...
class StatisticalTester:
    ...
    def run_all_tests(self, key: List[bool], key_length: int) -> Dict[str, int]:
        ...
        results = {
            ...
            'blind_spot_complexity': self.blind_spot_complexity_test_iterative(key)  # Iterative versiyonu kullan
        }
    ...
    def blind_spot_complexity_test(self, bits: List[bool]) -> int:
        """Blind spot complexity - RECURSIVE (eski versiyon)"""
        return self._blind_spot_recursive(bits, len(bits))
    ...
    def _blind_spot_recursive(self, bits: List[bool], length: int) -> int:
        """Recursive blind spot complexity (recursion limit problemli)"""
        if length == 1:
            return 1 if bits[0] else 0
        
        x = self._blind_spot_recursive(bits, length - 1)
