...
set key_count=0
for /f %%i in ('dir /b "output1\keys\*.txt" 2^>nul ^| find /c /v ""') do set key_count=%%i
...
set test_count=0
for /f %%i in ('dir /b "output1\test_results\*.csv" 2^>nul ^| find /c /v ""') do set test_count=%%i
...
for /f %%i in ('dir /b "output1\test_results\*.csv" 2^>nul ^| find /c /v ""') do set final_test_count=%%i

echo 📊 Sonuçlar:
echo   - Test sonucu dosyası: %final_test_count%/72
echo   - Klasör: output1\test_results\
echo   - Log: logs\testing.log

REM Final raporu kontrol et
if exist "output1\reports\detailed_experiment_results.csv" (
    echo   - Final rapor: output1\reports\detailed_experiment_results.csv
    echo.
    echo ✅ Analiz tamamlandı!
    echo.
    choice /C YN /M "Final raporunu Excel'de açmak istiyor musunuz"
    if !errorlevel!==1 (
        start excel "output1\reports\detailed_experiment_results.csv"
    )
) else (
